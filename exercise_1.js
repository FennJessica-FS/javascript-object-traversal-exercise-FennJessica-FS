...
(() => {
  ...
  //1. Array of all defenders with output to the console//
  const defenders = api.squad.filter(
    (player) => player.position === "Defender"
  );
  console.log("All Defenders:", defenders);

  //2. Array of all players sorted by id with output to the console //
  const playersSortedById = [...api.squad].sort((a, b) => a.id - b.id);
  console.log("Players sorted by ID:", playersSortedById);

  //3. statement with appropriate information for each [variable] below with output to the console //
  //"[TeamName] of [Location] remains in [rank] place in the [league]."//
  const teamName = api.name;
  const location = api.location;
  const rank = api.note.rank;
  const league = api.note.description;
  const statement = `${teamName} of ${location} remains in ${rank} place in the ${league}.`;
